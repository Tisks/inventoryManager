import { collection, FieldPath, query, where, WhereFilterOp, addDoc, setDoc, updateDoc, doc } from 'firebase/firestore';
...
export const useGetFromFirestore = (
    ...
    whereClause: [fieldPath: string | FieldPath, opStr: WhereFilterOp, value: unknown],
) => {
...
const submitRequest = async <T>(collectionName: TCollection, data: T, customId?: string) => {
    try {
        let res;
        if (customId) {
...
/* eslint-disable @typescript-eslint/no-explicit-any */
export const submitToFirestore = async (collectionName: TCollection, data: any, customId?: string) =>
    submitRequest(collectionName, data, customId);

export const useSubmitToFirestore = () => submitRequest;

/* eslint-disable @typescript-eslint/no-explicit-any */
const updateRequest = async <T>(collection: Collection, docId: string, data: T) => {
    try {
...
/* eslint-disable @typescript-eslint/no-explicit-any */
export const updateToFirestore = async <T>(collection: Collection, docId: string, data: T) =>
